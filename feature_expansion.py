...
class FeatureExpander(MessagePassing):
    ...
    def __init__(self, max_node_num, degree=True, onehot_maxdeg=0, AK=1,
    ...
    def transform(self, data):
        ...
        mask_batch = self.create_mask_batch(features.size(0), self.max_node_num)

        data.x, data.adj, data.mask = features_batch, adj_batch, mask_batch
        del data.edge_index
        ##########################################################################
        return data
    ...
    def create_adj_batch(self, adj, max_node_num):
        #adj_batch = torch.stack([create_adj((adj_batch[i]==1).nonzero(), max_node_num) for i in range(batch_size)], dim=0)
